 ...
 *
 * Від `Screen` відрізняється однією річчю: рубрики немає. У `Screen` вона
 * каже, у якому ти розділі; тут розділів немає, і стояла в ній назва
 * застосунку — тобто «slovnuk» набирався тим самим 10-піксельним капсом, що
 * й підпис поля «ПОШТА» під ним. Назва лишається там, де їй місце: у вкладці
 * браузера й на іконці.
 *
 * Нічого замість неї не з'явилось, і це рішення, а не недогляд: на дверях
 * п'ять предметів разом із заголовком, і будь-яка прикраса серед них стає
 * найгучнішою.
 *
 * `aside`, `back` і `foot` тут не передбачені навмисно: на дверях немає ні
 * рівня вище, ні панелі вкладок, ні профілю.
 */

...
class QueueResponseSchema(BaseModel):
    ...
    due_count: int = Field(description="Прострочені повторення — обидва види доріжок")
    new_count: int = Field(description="Нові доріжки — і перекладу, і форм")
    new_forms_count: int = Field(
        description=(
            "Скільки з new_count — доріжки форм. Підмножина, а не окрема купка: "
            "нових слів у черзі рівно new_count - new_forms_count."
        )
    )

...
export default function NoteEditScreen({ mode }: { mode: "create" | "edit" }) {
  ...
  const save = async () => {
    ...
    const problem = titleProblem(draft.title) ?? categoryProblem(draft.category);
    if (problem) {
      setError(problem);
      return;
    }
  ...
  const hints = categorySuggestions(categories.data?.items ?? [], draft.category);
  ...
  return (
    <div className="sheet-page">
      ...
      {hints.hidden > 0 ? (
        <div className="hint">
          Ще {hints.hidden} {plural(hints.hidden, "розділ", "розділи", "розділів")} —
          почніть набирати назву.
        </div>
      ) : null}
      ...
      {preview ? (
        ...
        <textarea
          ...
          rows={14}
          value={draft.body}
          placeholder={"Правило звичайним текстом.\n\n- пункт списку\n**жирний**  *курсив*\n# помітніший рядок"}
